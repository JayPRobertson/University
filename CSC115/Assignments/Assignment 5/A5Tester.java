public class A5Tester {
	...
	public static void testTotalBooks() {
		...
		LinkedBookList listN = new LinkedBookList();
		LinkedBookList listE = new LinkedBookList();
		LinkedBookList listX = new LinkedBookList();
		Book empty = new Book("", "", 0);
		Book[] none = {};
		Book[] booksEmpty = {empty};
		Book[] booksX = {empty, b4};
		listN.buildFromArray(none);
		listE.buildFromArray(booksEmpty);
		listX.buildFromArray(booksX);

		result = listN.totalBooks();
		expected = 0;
		displayResults(result==expected, "totalBooks in listN");

		result = listE.totalBooks();
		expected = 1;
		displayResults(result==expected, "totalBooks in listE");

		result = listX.totalBooks();
		expected = 2;
		displayResults(result==expected, "totalBooks in listX");
	...
	public static void testTotalPages() {
		...
		result = listN.totalPages();
		expected = 0;
		displayResults(result==expected, "totalPages in listN");
		
		result = listE.totalPages();
		expected = 0;
		displayResults(result==expected, "totalPages in listE");
		
		result = listX.totalPages();
		expected = 0+390;
		displayResults(result==expected, "totalPages in listX");
	...
	public static void testCountLongerThan() {
		...
		result = listN.countLongerThan(b2);
		expected = 0;
		displayResults(result==expected, "count books longer than b2 in listN");
		
		result = listE.countLongerThan(b3);
		expected = 0;
		displayResults(result==expected, "count books longer than b3 in listE");
		
		result = listX.countLongerThan(b4);
		expected = 0;
		displayResults(result==expected, "count books longer than b4 in listX");
	...
	public static void testPagesBeforeBookByAuthor() {
		...
		LinkedBookList listE = new LinkedBookList();
		LinkedBookList listX = new LinkedBookList();
		LinkedBookList listM = new LinkedBookList();

		Book empty = new Book("", "", 0);
		Book[] none = {};
		Book[] booksEmpty = {empty};
		Book[] booksX = {empty, b4};
		Book[] duplicate = {b4, b4, b4, b4, b4};
		listN.buildFromArray(none);
		listE.buildFromArray(booksEmpty);
		listX.buildFromArray(booksX);
		listM.buildFromArray(duplicate);

		result = listN.pagesBeforeBookByAuthor("Gillian Flynn");
		expected = 0;
		displayResults(result==expected, "pagesBeforeBookByAuthor Gillian Flynn in listN");
		
		result = listX.pagesBeforeBookByAuthor("Suzanne Collins");
		expected = 0;
		displayResults(result==expected, "pagesBeforeBookByAuthor Suzanne Collins in listX");
		
		result = listE.pagesBeforeBookByAuthor("Fonda Lee");
		expected = 0;
		displayResults(result==expected, "pagesBeforeBookByAuthor Fonda Lee in listE");

		result = listM.pagesBeforeBookByAuthor("Suzanna Collins");
		expected = 390*5;
		displayResults(result==expected, "pagesBeforeBookByAuthor Fonda Lee in listM");
	...
	public static void testLongestBook() {
		...
		result = listN.longestBook();
		displayResults(result == null, "longest book in listN");
		
		result = listE.longestBook();
		expected = empty;
		displayResults(result.equals(expected), "longest book in listE");
		
		result = listX.longestBook();
		expected = b4;
		displayResults(result.equals(expected), "longest book in listX");

		result = listM.longestBook();
		expected = b4;
		displayResults(result.equals(expected), "longest book in listM");
	...
	public static void testDistanceBetweenBooksByAuthor() {
		...
		LinkedBookList listM = new LinkedBookList();
		LinkedBookList listS = new LinkedBookList();
		Book[] duplicate = {b4, b4, b4, b4, b4};
		Book[] space = {b4, b2, b4, b4};
		listM.buildFromArray(duplicate);
		listS.buildFromArray(space);

		result = listM.distanceBetweenBooksByAuthor("Suzanne Collins");
		expected = 0;
		displayResults(result==expected, "distance between books by Suzanne Collins in listM");

		result = listS.distanceBetweenBooksByAuthor("Suzanne Collins");
		expected = 1;
		displayResults(result==expected, "distance between books by Suzanne Collins in listS");
	...
	public static void testCountMatches() {
		...
		SinglyLinkedList<Double> listF = new SinglyLinkedList<Double>();
		Double[] arrF = {1.2, 0.0, 1.1, 1.0, 1.00001, 1.000001, 1.01, 1.1};
		listF.buildFromArray(arrF);

		result = A5Exercises.countMatches(listF, 1.00001);
		expected = 1;
		displayResults(result==expected, "countMatches, one found");

		result = A5Exercises.countMatches(listF, 1.1);
		expected = 2;
		displayResults(result==expected, "countMatches, two found");
