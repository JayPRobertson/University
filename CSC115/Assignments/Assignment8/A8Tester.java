...
public class A8Tester {
	...
	static void test_insert_find()	{
		...
		try {
			...
			displayResults(s.equals("datawilma"), "find - should be there");
		} catch (KeyNotFoundException e) {
			displayResults(false, "find - should be there");
		}
		...
		try {
			...
			displayResults(q.equals("datacandy"), "find - should be there");
		} catch (KeyNotFoundException e) {
			displayResults(false, "find - should be there");
		}
	...
	public static void map1_test() {
		...
		boolean isTrue = d.containsKey("xyz");
		displayResults(isTrue ==true, "containsKey");
		
		isTrue = d.containsKey("lmno");
		displayResults(isTrue ==true, "containsKey");

		isTrue = d.containsKey("pqr");
		displayResults(isTrue ==true, "containsKey");

		isTrue = d.containsKey("ghi");
		displayResults(isTrue ==false, "containsKey");

		d.clear();
		displayResults(d.size() == 0, "clear + size");

		d.put("xyz", 11);
		d.put("lmno", 1);
		d.put("pqr", 3);

		displayResults(d.size() == 3, "clear + size");

		try{
			int result = d.get("pqr");
			displayResults(result == 3, "get");	
		}catch(KeyNotFoundException e){
			displayResults(false, "get");	
		}
		
		try{
			d.put("pqr", 21);
			int result = d.get("pqr");
			displayResults(result == 21, "get");	
		}catch(KeyNotFoundException e){
			displayResults(false, "get");	
		}
