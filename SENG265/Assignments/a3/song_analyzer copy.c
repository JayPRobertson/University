...
node_t* readlines(FILE* filename, char* order_by, char* order, char* filter, char* val){
    ...
    char* token; char* song; char* artists; char* order_val;
    ...
    while(fgets(line, MAX_LINE_LEN, filename) != NULL){  
        ...
        //get values needed to make node
        song = strtok(line, CSV_SEPARATE);
        artists = strtok(NULL, CSV_SEPARATE);
        token = strtok(NULL, CSV_SEPARATE);
        year = atoi(strtok(NULL, CSV_SEPARATE));
        month = atoi(strtok(NULL, CSV_SEPARATE));
        day = atoi(strtok(NULL, CSV_SEPARATE));
        
        //determine what column to keep for "order" param in node
        if (!strcmp(order_by, "STREAMS")){
            token = strtok(NULL, CSV_SEPARATE);
            order_val = strtok(NULL, CSV_SEPARATE); //get streams
        }else if (!strcmp(order_by, "NO_SPOTIFY_PLAYLISTS")){
            order_val = strtok(NULL, CSV_SEPARATE); //get in_spotify_playlists
        }else{
            token = strtok(NULL, CSV_SEPARATE);
            token = strtok(NULL, CSV_SEPARATE);
            order_val = strtok(NULL, CSV_SEPARATE); //get in_apple_playlists
        }
        
        //make node
        node_t* cur_node = new_node(song, artists, year, month, day, order_val);
